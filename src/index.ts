import { Client, GatewayIntentBits, Events, DefaultWebSocketManagerOptions, Message, EmbedBuilder, codeBlock, ButtonBuilder, ButtonStyle, ActionRowBuilder, ComponentType, ButtonInteraction } from 'discord.js';
...
client.on(Events.MessageCreate, async function (message: Message) {
  ...
  let outputString = ("object" === typeof outputCode ? inspect(outputCode) : outputCode) + "";
  ...
  for(let i = 0; i < outputChunks.length; i++) {
    ...
    let embed = new EmbedBuilder().setColor(0x313336).setTitle("Evaluated").setFooter({ text: `Page ${i + 1}/${outputChunks.length} • ${outputString.length} • ${typeof outputCode}` }).setDescription(codeBlock("js", chunk));
    embeds.push(embed);
  }
  ...
  function addComponents(...available: boolean[]) {
    return [
      ...
      .addComponents(
        new ButtonBuilder().setEmoji("⏪").setCustomId("first").setStyle(ButtonStyle.Secondary).setDisabled(available[0]),
        new ButtonBuilder().setEmoji("◀️").setCustomId("previous").setStyle(ButtonStyle.Secondary).setDisabled(available[1]),
        new ButtonBuilder().setEmoji("🗑️").setCustomId("delete").setStyle(ButtonStyle.Danger).setDisabled(available[2]),
        new ButtonBuilder().setEmoji("▶️").setCustomId("next").setStyle(ButtonStyle.Secondary).setDisabled(available[3]),
        new ButtonBuilder().setEmoji("⏩").setCustomId("last").setStyle(ButtonStyle.Secondary).setDisabled(available[4])
      )
    ]
  }
